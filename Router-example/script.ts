...
<div class="list-group">
  <a [routerLink]="['/artist', track.artistId, track.trackId]"
...
@Component({
  ...
  template: `
<div class="container">
  <div class="row" *ngIf="track">
    <div class="col-xs-4 col-sm-2">
    <img class="img-rounded img-responsive" alt={{track.collectionName}} src="{{track.artworkUrl100}}">
    </div>
    <div class="col-xs-8 col-sm-10">
    <h3>{{track.trackName}} <small> {{track.artistName}}</small></h3>
    <p>Album: {{track.collectionName}}</p>
    <p>Release Date: {{track.releaseDate | date}}</p>
    </div>
...
class ArtistTrackComponent {
  private track: any;

  constructor(private jsonp: Jsonp,
              private route: ActivatedRoute) {
    this.route.parent.params.subscribe(params => {
      this.jsonp.request(`https://itunes.apple.com/lookup?id=${params['trackId']}&callback=JSONP_CALLBACK`)
          .toPromise()
          .then(res => {
            console.log(res.json());
            this.track = res.json().results[0];
          });
    });
  }
}
...
const routes: Routes = [
  ...
  {
    path: 'artist/:artistId/:trackId',
    component: ArtistComponent,
    children: [
      {path: '', redirectTo: 'track', pathMatch: 'full'},
      {path: 'track', component: ArtistTrackComponent},
